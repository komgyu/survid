...
def main():
    ...
    def computeIoU(scores, labels):
        ...
        count = np.zeros([480,256],dtype = np.uint8)
        count = np.logical_and(labels[:,:]==0, scores[:,:]==0)
        num00 = np.count_nonzero(count)
    
        count = np.zeros([480,256],dtype = np.uint8)
        count = np.logical_and(labels[:,:]==0, scores[:,:]==1)
        num01 = np.count_nonzero(count)

        count = np.zeros([480,256],dtype = np.uint8)
        count = np.logical_and(labels[:,:]==0, scores[:,:]==2)
        num02 = np.count_nonzero(count)

        count = np.zeros([480,256],dtype = np.uint8)
        count = np.logical_and(labels[:,:]==1, scores[:,:]==0)
        num10 = np.count_nonzero(count)

        count = np.zeros([480,256],dtype = np.uint8)
        count = np.logical_and(labels[:,:]==1, scores[:,:]==1)
        num11 = np.count_nonzero(count)

        count = np.zeros([480,256],dtype = np.uint8)
        count = np.logical_and(labels[:,:]==1, scores[:,:]==2)
        num12 = np.count_nonzero(count)

        count = np.zeros([480,256],dtype = np.uint8)
        count = np.logical_and(labels[:,:]==2, scores[:,:]==0)
        num20 = np.count_nonzero(count)

        count = np.zeros([480,256],dtype = np.uint8)
        count = np.logical_and(labels[:,:]==2, scores[:,:]==1)
        num21 = np.count_nonzero(count)

        count = np.zeros([480,256],dtype = np.uint8)
        count = np.logical_and(labels[:,:]==2, scores[:,:]==2)
    ...
    while epoch<= 500:
        for i, data in enumerate(trainloader):
            ...
            for i in range(scores.size()[0]):
                current_iou = current_iou + computeIoU(s[i].unsqueeze(0), s_gt[i].unsqueeze(0))

            train_iou = train_iou*0.99 + current_iou*0.01/scores.size()[0]
