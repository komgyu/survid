...
class ValDataSet(torch.utils.data.Dataset):
	def __init__(self, root, list_path, ignore_label=255,device ='cuda'):
		...
		self.files = []
		for name in self.img_ids:
			img_file = os.path.join(self.root, "validate/2peg/scale/original/%s.png" % name)
			label_file = os.path.join(self.root, "validate/2peg/scale/label/%s.png" % name)
			flow_file = os.path.join(self.root, "validate/2peg/scale/flow/%s.png" % name)
			self.files.append({
				"img": img_file,
				"label": label_file,
				"flow": flow_file,
