...
class SNet(torch.nn.Module):
    def __init__(self, nch):
        ...
        self.conv10 = torch.nn.Conv2d(32, 3, 1, padding = 0) #kernel size = 1
    ...
    def forward(self, x):
        ...
        up1   = self.up1(hh5)
        cat1  = torch.cat([up1, hh4],dim = 1) # 256+256
        hh6   = self.conv6(cat1)
        up2   = self.up2(hh6)
        cat2  = torch.cat([up2, hh3], dim = 1) #128 + 128
        hh7   = self.conv7(cat2)
        up3   = self.up3(hh7)
        cat3  = torch.cat([up3, hh2], dim = 1) #64 + 64
        hh8   = self.conv8(cat3)
        up4   = self.up4(hh8)  
        cat4  = torch.cat([up4, hh1], dim = 1) #32 + 32
        hh9   = self.conv9(cat4)
        hh   = self.conv10(hh9)
